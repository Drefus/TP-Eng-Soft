...
public class WorldCupApiSyncService {
    ...
    private Selecao getOrCreateTeam(String name, String groupLabel) {
        ...
        s.setNome(name);
        // If it's a "TBD" or "Winner", don't assign a group to avoid cluttering the group stage tables
        if (name.startsWith("Winner") || name.startsWith("Runner") || name.startsWith("Loser") || name.equals("TBD") || name.startsWith("3rd")) {
            s.setGrupo("??");
        } else {
            // "group" in API could be "A", "B", etc. Only use first letter if it's a group stage.
    ...
    private String mapPhase(String type, String groupLabel) {
        if ("group".equalsIgnoreCase(type)) return "Grupo";
        if ("r32".equalsIgnoreCase(type)) return "16-avos";
        if ("r16".equalsIgnoreCase(type)) return "Oitavas";
        if ("qf".equalsIgnoreCase(type)) return "Quartas";
        if ("sf".equalsIgnoreCase(type)) return "Semifinal";
        if ("third".equalsIgnoreCase(type)) return "Terceiro Lugar";
        if ("final".equalsIgnoreCase(type)) return "Final";
    ...
    private void updateMatchScoreAndStatus(Partida p, WorldCupGameDto apiMatch) {
        if (apiMatch.getHomeScore() == null || apiMatch.getAwayScore() == null) return;
        if ("null".equals(apiMatch.getHomeScore()) || "null".equals(apiMatch.getAwayScore())) return;
